...
# Users parameters
days_to_simulate = st.slider('Days to Simulate', min_value=365, max_value=365*50, value=365*30, step=365)
n_iteration = st.slider('Number of Iterations', min_value=1, max_value=1000, value=1)
yearly_return_on_investment = st.slider('Yearly Return on Investment', min_value=0.0, max_value=0.20, value=0.05, step=0.01)
avg_daily_mast_on_taking_money = st.slider('Average Daily Mast on Taking Money', min_value=0.0, max_value=1.0, value=0.01)
avg_daily_mast_not_taking_money = st.slider('Average Daily Mast Not Taking Money', min_value=0.0, max_value=5.0, value=2.0)
...
for simulation in range(n_iteration):
    ...
    for day in range(days_to_simulate):
        if np.random.rand() < avg_daily_mast_on_taking_money:
            money_taking_balance -= EVERY_MAST_LOSS
        if invest_the_money:
            money_taking_balance += money_taking_balance * daily_interest_rate
        all_taking_money_history[simulation, day] = money_taking_balance

        if np.random.rand() < avg_daily_mast_not_taking_money:
            not_taking_money_balance += EVERY_MAST_WIN
        if invest_the_money:
            not_taking_money_balance += not_taking_money_balance * daily_interest_rate
        all_not_taking_money_history[simulation, day] = not_taking_money_balance
...
plt.title('Average Balance Over Time Across Mast Simulations')
plt.legend()
st.pyplot(plt)
